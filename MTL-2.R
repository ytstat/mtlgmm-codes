...
filename <- paste("/home/yt2170/work/mtlgmm/experiments/simulation_test_dist_beta/result/", seed, ".RData", sep = "")
...
error_single <- matrix(0, nrow = nrow(C_matrix), ncol = 5, dimnames = list(NULL, c("w", "mu", "beta", "Sigma", "prediction")))
error_mtlgmm <- matrix(0, nrow = nrow(C_matrix), ncol = 5, dimnames = list(NULL, c("w", "mu", "beta", "Sigma", "prediction")))
error_pooling <- matrix(0, nrow = nrow(C_matrix), ncol = 5, dimnames = list(NULL, c("w", "mu", "beta", "Sigma", "prediction")))
...
for (i in 1:nrow(C_matrix)) {
  ...
  while(flag) {
    print(i)
    data_bundle <- data_generation(K = 10, eps = 0.5, outlier_K = C_matrix[i, "outlier_num"], outlier_type = "dist", h_w = C_matrix[i, "h_w"], h_mu = C_matrix[i, "h_mu"], n = 600, generate_type = "beta_new")

    # data_bundle <- data_generation(K = 10, outlier_K = 2, outlier_type = "dist", h_w = 0.05, h_mu = 0.4, n = 600, generate_type = "beta_new")
